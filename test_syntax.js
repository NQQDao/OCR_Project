        ...
        function renderHistoryTable(docs, startIndex = 0) {
            ...
            tbody.innerHTML = docs.map((doc, idx) => {
                const thumbHtml = doc.image_path
                    ? `<img src="/web_uploads/${encodeURIComponent(doc.image_path)}" class="thumb-mini" alt="Ảnh phiếu" onerror="this.onerror=null; this.src='data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'40\' height=\'40\' viewBox=\'0 0 40 40\'><rect width=\'40\' height=\'40\' fill=\'%23f1f5f9\'/><text x=\'50%\' y=\'50%\' font-family=\'Arial\' font-size=\'10\' fill=\'%2394a3b8\' text-anchor=\'middle\' dy=\'.3em\'>Lỗi</text></svg>';" title="Bấm xem to" onclick="openFullscreenModal('/web_uploads/${encodeURIComponent(doc.image_path)}')">`
                    : `<div class="thumb-placeholder" title="Không có ảnh">📄</div>`;

                const timeStr = doc.created_at
                    ? new Date(doc.created_at.replace(" ", "T") + "Z").toLocaleString("vi-VN", { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })
                    : "-";
        ...
        function renderR2FilesHistoryTable(files) {
            ...
            tbody.innerHTML = files.map((f, idx) => {
                ...
                if (f.file_type === 'excel') {
                    actionBtns = `
                        <button type="button" class="btn-action-sm" onclick="openExcelPreviewModal('${escapeHtml(f.public_url)}')" style="background: #eff6ff; color: #1d4ed8; border-color: #bfdbfe; margin-right: 4px;" title="Xem trước file Excel trực tiếp trên trình duyệt">
        ...
        function applyBatchImageTransform(isInstant = false) {
            ...
            img.style.transform = `translate(${batchImgPanX}px, ${batchImgPanY}px) scale(${batchImgScale}) rotate(${batchImgRotation}deg)`;
